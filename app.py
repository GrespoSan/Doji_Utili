...
with st.sidebar:
    ...
    st.info("Nota: Se non trovi nulla, aumenta la tolleranza (es. 0.30/0.35).")
...
if 'results' in st.session_state:
    ...
    if len(res_list) > 0:
        ...
        # ELIMINAZIONE RANGESLIDER
        fig.update_layout(
            template="plotly_dark", 
            title=f"{ticker_to_show} - Prossimi Utili: {selected_data['Data Utili']}",
            xaxis_rangeslider_visible=False,  # Questa riga rimuove il grafico sotto
            height=600
        )
